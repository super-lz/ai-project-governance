...
2. 内部运行 `scripts/manage_norn_governance.py analyze --target <repo> --report-json`。分析不得写入目标仓库；机器计划和渲染产物留在系统临时目录。
   ...
   - 用户明确要求完整迁移旧 `appendix/`、`spec/` 或两者时，才在内部分析命令加入对应的 `--include-legacy-tree appendix|spec|all`。先列出实际文件范围，不把参数转嫁给用户。
...
- 默认只迁移确认属于旧 Norn 的四个核心 `docs/` 文件。用户明确授权扩大范围后，可递归迁移 `docs/spec/**`、`docs/appendix/**` 或两者；`docs/` 的其他内容仍不移动。
- 显式范围决定哪些文件获得迁移授权，不要求额外文件匹配旧模板哈希。文件哈希只用于绑定计划、检测确认后的变化和安全判断目标是否相同。
- 显式迁移的普通文件保持原始字节；目标内容不同、父路径冲突、符号链接或特殊文件必须报告并阻断，不能静默覆盖或跟随。
