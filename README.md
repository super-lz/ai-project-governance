...
默认迁移保持最小范围，只处理上面的四个核心文件。用户明确要求完整迁移旧治理内容时，Skill 可以进一步递归迁移 `docs/spec/**`、`docs/appendix/**` 或两者，并在确认前列出实际文件清单。额外普通文件按原始字节迁移；Hash 只用于计划一致性、变更检测和目标去重，不作为用户已授权文件的归属门槛。目标冲突、符号链接和特殊文件仍会阻断，`docs/` 其他区域始终不受影响。
